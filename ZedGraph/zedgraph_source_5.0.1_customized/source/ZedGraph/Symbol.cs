...
namespace ZedGraph
{
	...
	/// <version> $Revision: 5 $ $Date: 5/14/10 11:20a $ </version>
	[Serializable]
	...
	#region Rendering Methods
		...
		/// <param name="isHighlighted">whether the symbol is highlighted (passed on from the parent CurveItem
		/// highlighted state).</param>
		/// <param name="penWidth">width of pen to be used to draw this path.  The endpoint of the path will be 
		/// extended so the symbol looks ok with any pen width.</param>
		/// <returns>Returns the <see cref="GraphicsPath"/> for the current symbol</returns>
		public GraphicsPath MakePath( Graphics g, float scaleFactor, bool isHighlighted, float penWidth )
		{
			...
			case SymbolType.Square:
				...
				path.AddLine( hsize, -hsize, hsize, hsize );
				path.AddLine( hsize, hsize, -hsize, hsize );
				path.AddLine(-hsize, hsize, -hsize, -hsize - penWidthMod);
				break;
			case SymbolType.Diamond:
				path.AddLine( 0, -hsize, hsize, 0 );
				path.AddLine( hsize, 0, 0, hsize );
				path.AddLine( 0, hsize, -hsize, 0 );
				path.AddLine( -hsize, 0, 0 + penWidthMod * .75f, -hsize - penWidthMod * .75f );
				break;
			case SymbolType.Triangle:
				path.AddLine( 0, -hsize, hsize, hsize );
				path.AddLine( hsize, hsize, -hsize, hsize );
				path.AddLine(-hsize, hsize, 0 + penWidthMod * .5f, -hsize - penWidthMod * .5f);
				break;
			case SymbolType.Circle:
				path.AddEllipse( -hsize, -hsize, scaledSize, scaledSize );
				break;
			case SymbolType.XCross:
				path.AddLine(-hsize - penWidthMod, -hsize - penWidthMod, hsize1 + penWidthMod, hsize1 + penWidthMod);
				path.StartFigure();
				path.AddLine(hsize + penWidthMod, -hsize - penWidthMod, -hsize1 - penWidthMod, hsize1 + penWidthMod);
				break;
			case SymbolType.Plus:
				path.AddLine(0, -hsize - penWidthMod, 0, hsize1 + penWidthMod);
				path.StartFigure();
				path.AddLine(-hsize - penWidthMod, 0, hsize1 + penWidthMod, 0);
				break;
