...
namespace ZedGraph
{
	...
	/// <version> $Revision: 7 $ $Date: 5/14/10 11:20a $ </version>
	[Serializable]
	public class Bar : ICloneable, ISerializable
	{
		...
		/// <summary>
		/// rectangles, in pixel coordinates, occupied by each bar.
		/// </summary>
		private List<RectangleF> _barRectangles = new List<RectangleF>();
		/// <summary>
		/// indexes to highlight if highlighted flag is true.
		/// </summary>
		private List<int> _highlightedBarIndexes = new List<int>();
		// end BioRad
		...
		// BioRad addition 7.9.2007
		/// <summary>
		/// highlighted bar indexes.
		/// </summary>
		public List<int> HighlightedBarIndexes
		{
			get { return _highlightedBarIndexes; }
			set { _highlightedBarIndexes = value; }
		}
		// end BioRad
		/// <summary>
		...
		#region methods
		/// <summary>
		/// find closest bar in this series of bars to a given point in pixel space.
		/// </summary>
		/// <param name="fromPixel">point from which to measure.</param>
		/// <param name="maxDistance">max distance to consider</param>
		/// <returns>object containing info about the closest bar, or null if there is no closest bar
		/// within the max distance.</returns>
		internal virtual BarItem.ClosestBarInfo FindClosestBar(Point fromPixel, float maxDistance)
		{
			BarItem.ClosestBarInfo closestBar = null;
			for (int barIndex = 0; barIndex < _barRectangles.Count; barIndex += 1)
			{
				RectangleF barRectangle = _barRectangles[barIndex];
				float xDist = 0;
				if (fromPixel.X < barRectangle.Left || fromPixel.X > barRectangle.Right)
				{
					xDist = Math.Min(
							 Math.Abs(fromPixel.X - barRectangle.Left),
							 Math.Abs(fromPixel.X - barRectangle.Right));
				}
				float yDist = 0;
				Debug.Assert(barRectangle.Top <= barRectangle.Bottom, "inverted rectangle?");
				if (fromPixel.Y < barRectangle.Top || fromPixel.Y > barRectangle.Bottom)
				{
					yDist = Math.Min(
							 Math.Abs(fromPixel.Y - barRectangle.Top),
							 Math.Abs(fromPixel.Y - barRectangle.Bottom));
				}

				float closestDist = Math.Max(xDist, yDist);
				bool isClosest = false;
				if (closestDist <= maxDistance)
				{
					if (closestBar != null)
					{
						if (closestDist < closestBar.Distance)
							isClosest = true;
					}
					else
						isClosest = true;
				...
				{
					if (closestBar == null)
						closestBar = new BarItem.ClosestBarInfo();
					closestBar.BarIndex = barIndex;
					closestBar.Distance = closestDist;
				}
			}
