...
namespace ZedGraph
{
	...
	/// <version> $Revision: 6 $ $Date: 7/08/09 4:45p $ </version>
	[Serializable]
	...
	#region properties
		...
		// begin biorad change: removed some overrides which floor values at zero, this is taken care of by MinLogDisplayValue.
#if false
		/// <summary>
	...
	#region methods
		...
		override internal double CalcBaseTic()
		{
			...
			else
			{
				...
				return Math.Ceiling( SafeLog10( _min ) - 0.00000001 );
			}
		...
		override internal int CalcNumTics()
		{
			...
			nTics = (int) ( ( Math.Floor( SafeLog10( _max ) + 1.0e-12 ) ) -
					( Math.Ceiling( SafeLog10( _min ) - 1.0e-12 ) ) + 1 ) / CyclesPerStep;
		...
		{
			// Change by BioRad 2007.01.01 : Changed Scale.SafeLog method call to Scale.SafeLog10.
			get { return (int)Math.Max( Math.Floor( SafeLog10( _majorStep ) ), 1 ); }
			// End BioRad change.
		}
