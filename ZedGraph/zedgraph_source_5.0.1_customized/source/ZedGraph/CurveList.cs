...
namespace ZedGraph
{
	...
	/// <version> $Revision: 5 $ $Date: 5/08/08 4:56p $ </version>
	[Serializable]
...
*/
	...
	#region Rendering Methods
		...
		public void Draw( Graphics g, GraphPane pane, float scaleFactor )
		{
			...
			// Added by BioRad 2007.0.01
			List<CurveItem> highlightedCurves = new List<CurveItem>();
			// End BioRad addition
			...
			for ( int i=this.Count-1; i>=0; i-- )
			{
				...
				if (!(curve.IsBar && pane._barSettings.Type == BarType.SortedOverlay))
				{
					...
					{
						// draw highlighted ones last...
						highlightedCurves.Add(curve);
						continue;
					}
					// End BioRad addition
			...
			// Added by BioRad 2007.0.01
			foreach (CurveItem curve in highlightedCurves)
			{
				curve.Draw(g, pane, pos, scaleFactor);
			}
			// End BioRad addition
