...
namespace ZedGraph
{
	...
	/// <version> $Revision: 2 $ $Date: 7/11/07 2:34p $ </version>
	[Serializable]
	...
	#region Methods
		...
		/// </param>
		/// <param name="isHighlighted">whether the border is highlighted (increases width)</param>
		...
		public Pen MakePen(bool isPenWidthScaled, float scaleFactor)
		{
			return MakePen(isPenWidthScaled, scaleFactor, false);
		}
		// END  BioRad Change
		...
		/// <param name="isHighlighted">whether this border is highlighted (increases width)</param>
			...
			)
		{
			...
			if ( _isVisible )
			{
				...
				// Changed by BioRad 7.11.2007 - replaced line
				//g.DrawRectangle(MakePen(isPenWidthScaled, scaleFactor), tRect.X, tRect.Y, tRect.Width, tRect.Height);
				g.DrawRectangle(MakePen(isPenWidthScaled, scaleFactor, isHighlighted), tRect.X, tRect.Y, tRect.Width, tRect.Height);
				// END  BioRad Change
			}
		}
		...
		public void Draw(Graphics g, bool isPenWidthScaled, float scaleFactor, RectangleF rect)
		{
			this.Draw(g, isPenWidthScaled, scaleFactor, rect, false);
		}
		// END  BioRad Change
	#endregion
	}
