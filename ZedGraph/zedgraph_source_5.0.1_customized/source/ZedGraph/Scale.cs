...
namespace ZedGraph
{
	...
	/// <version> $Revision: 13 $ $Date: 7/08/09 4:45p $ </version>
	[Serializable]
	abstract public class Scale : ISerializable
	{
		...
		// Biorad addition
		/// <summary>
		/// Set to true for x-axis scales if you want left-justified angled labels.  This is a hack
		/// and will likely not work in other circumstances.  Only x axis.
		/// </summary>
		private bool _HACK_useAngledLabels = false;
		// end biorad

		// Biorad addition
		private double _minAutoRange = 0;
		/// <summary>Min range when in auto-scale mode.</summary>
		public double MinAutoRange
		{
			get { return _minAutoRange; }
			set { _minAutoRange = value; }
		}
		// end biorad
		...
		{
			// Added by BioRad 2007.3.2
			/// <summary>
			/// Minimum linear value to display on log-space scales.  Any value less than this
			/// will be floored to this for display purposes.  (Note, the data is not changed however.)
			/// </summary>
			public static double MinLogValue = .1;
			// End BioRad Addition
			/// <summary>
			...
			/// </summary>
			public static double ZeroLever = 0.05;
		...
		public Scale(Axis ownerAxis)
		{
			...
			_minLogValue = Scale.Default.MinLogValue;
			_min = 0.0;
			_max = 1.0;
			_forcedMin = _min;
			_forcedMax = _max;
		...
		public Scale(Scale rhs, Axis owner)
		{
			...
			_minLogValue = rhs._minLogValue;
			_min = rhs._min;
			_max = rhs._max;
			_forcedMax = rhs._forcedMax;
			_forcedMin = rhs._forcedMin;
			...
			// BioRad
			_HACK_useAngledLabels = rhs._HACK_useAngledLabels;
			// End BioRad.
		}
		...
		*/
		...
		public Scale MakeNewScale(Scale oldScale, AxisType type)
		{
			switch (type)
			{
				...
				// Added by BioRad 2007.01.01
				case AxisType.Log2:
					return new Log2Scale(oldScale, _ownerAxis);
				// End BioRad Addition.
		...
		#region properties
		/// <summary>
		/// Forced scale min.  Applied min is still bounded by MinLogDisplayValue regardless of this value.
		/// </summary>
		public double ForcedMin
		{
			get { return _forcedMin; }
			set { _forcedMin = value; }
		}
		/// <summary>
		/// Forced scale max.  Applied max is still bounded by MinLogDisplayValue regardless of this value.
		/// </summary>
		public double ForcedMax
		{
			get { return _forcedMax; }
			set { _forcedMax = value; }
		}
		// Added by BioRad 2007.03.13
		/// <summary>
		///Minimum value (linear space value) which will be displayed in log-space.  This is similar to setting
		/// the scale's min value to this value.
		/// </summary>
		public double MinLogDisplayValue
		{
			get { return _minLogValue; }
			set { _minLogValue = value; }
		}
		// End BioRad addition.
		...
		*/
		...
		/// default value for this property is see PointPair.Missing, which means the
		/// value is not used.  Setting this property to any value other than
		/// see PointPair.Missing will activate the effect.  The value specified must
		...
		/// and will likely not work in other circumstances.  Only x axis.
		/// </summary>
		public bool HACK_useAngledLabels
		{
			get { return _HACK_useAngledLabels; }
			set { _HACK_useAngledLabels = value; }
		}
		// End biorad.
		#endregion
		/*
...
*/
		...
		*/
			...
			for (int i = firstTic; i < nTics + firstTic; i++)
			{
				...
				if (_isVisible && !isSkipZone)
				{
					...
					if (IsPreventLabelOverlap &&
						Math.Abs(pixVal - lastPixVal) < maxLabelSize.Width * (this is Log2Scale ? 1.5 : 1))
						continue;
						...
						float shift, float maxSpace, float scaledTic, float charHeight, float scaleFactor)
		{
			...
			// Changed by BioRad 2007.01.01:
			//  Changed from DrawTenPower to DrawPower, added Log2Scale condition.

			//if ( this.IsLog && _isUseTenPower )
			if (this is LogScale)
			{
				_fontSpec.DrawPower(g, pane, tmpStr,
					pixVal, textCenter,
					AlignH.Center, AlignV.Center,
					scaleFactor, 10);
			}
			else if (this is Log2Scale)
			{
				_fontSpec.DrawPower(g, pane, tmpStr,
					pixVal, textCenter,
					AlignH.Center, AlignV.Center,
		...
		virtual public void PickScale(GraphPane pane, Graphics g, float scaleFactor)
		{
			...
			// Added by BioRad 2007.01.01
			double logspaceRange = 0;
			if (IsLog)
				logspaceRange = Linearize(maxVal) - Linearize(minVal);
			// End BioRad addition.
			...
			// Changed by BioRad 2007.01.01 : support for log scales. 
			if (_minAuto)
			{
				_min = minVal;
				...
				{
					// Add grace for log scale:
					// A note on the comments below: log-space is linear wrt our value axis, thus the Linearize()
					//  method tranforms to log-space.

					// Transform minVal to log-space
					double logspaceMinVal = Linearize(minVal);
					// Apply "zero-lever"
					if (Math.Abs(logspaceMinVal) < .001)
					{
						logspaceMinVal = 0;
					}
					else
					{
						// multiply log-space-range by _minGrace and subtract from log-space-minVal
						logspaceMinVal -= logspaceRange * _minGrace;
					}
					// tranform the result back to non-log space, set _min to this value.
					_min = DeLinearize(logspaceMinVal);
				}
			}
			else
			{
			...
			if (_maxAuto)
			{
				...
				{
					// Transform maxVal to log-space
					double logspaceMaxVal = Linearize(maxVal);
					// Apply "zero-lever"
					if (Math.Abs(logspaceMaxVal) < .001)
					{
						logspaceMaxVal = 0;
					}
					else
					{
						// multiply log-space-range by the grace and add to log-space-maxVal
						logspaceMaxVal += logspaceRange * _maxGrace;
					}
					// tranform the result back to non-log space, set _min to this value.
					_max = DeLinearize(logspaceMaxVal);
				}
			}
			else
			{
			...
			{
				if (_max - _min < _minAutoRange)
				{
					double currentRange = _max - _min;
					double additionalRangeNecessary = _minAutoRange - currentRange;
					_max += additionalRangeNecessary / 2;
					_min -= additionalRangeNecessary / 2;
				}
			}
			// end biorad.
		}
		...
		internal void SetRange(GraphPane pane, Axis axis)
		{
			if (_rangeMin >= Double.MaxValue || _rangeMax <= Double.MinValue)
			{
				...
				{
					// Change by BioRad 2007.2.9
					//  These magic numbers were causing problems for data which is between 0 and 1.
					//_rangeMin = 0;
					//_rangeMax = 1;
					// End BioRad change.
				}
		...
		/// <remarks>Changed by BioRad 2007.01.01 : changed name from SafeLog to SafeLog10.</remarks>
		/// <returns>The value of the logarithm, or 0 if the <paramref name="x"/>
		/// argument was negative or zero</returns>
		public double SafeLog10(double x)
		{
			// Added by BioRad 2007.03.01
			if (x < MinLogDisplayValue && _minAuto)
				x = MinLogDisplayValue;
			// End BioRad addition.

			return Math.Log10(Math.Max(x, 1.0e-20));
		}

		// Added by BioRad 2007.01.01
		/// <summary>
		/// SafeLog for arbitrary base
		/// </summary>
		/// <param name="x"></param>
		/// <param name="powerBase"></param>
		/// <returns></returns>
		public double SafeLog(double x, double powerBase)
		{
			if (x < MinLogDisplayValue && _minAuto)
				x = MinLogDisplayValue;

			return Math.Log(Math.Max(x, 1.0e-20), powerBase);
		}
		// End BioRad addition.
		...
		{
			return Math.Pow(Math.Max(x, 1.0e-20), exponent);
		}
