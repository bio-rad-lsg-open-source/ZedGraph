...
namespace ZedGraph
{
	...
	/// segments that make up a curve on the graph, optimized for fast drawing.
	/// </summary>
	/// 
	/// <author> John Champion 
	/// This file was added by Biorad.  It is based on Line.cs</author>
	/// <version> $Revision: 9 $ $Date: 5/14/10 11:20a $ </version>
	[Serializable]
	public class FastLine : ICloneable, ISerializable
	{
		#region Fields
		/// <summary>
		/// Private field that stores the pen width for this
		/// <see cref="FastLine"/>.  Use the public
		/// property <see cref="Width"/> to access this value.
		/// </summary>
		private float _width;
		// BioRad
		/// <summary>
		/// width when pen is highlighted, not counting black border.
		/// </summary>
		private float _highlightedWidth;
		// End BioRad
		/// <summary>
		...
		/// for this <see cref="FastLine"/>.  Use the public property
		...
		/// <see cref="FastLine"/>.  Use the public property <see cref="Fill"/> to
		/// access this value.
		/// </summary>
		private Fill _fill;
		...
		/// default property values for the <see cref="FastLine"/> class.
		/// </summary>
		public struct Default
		{
			// Default FastLine properties
			/// <summary>
			/// The default color for curves (line segments connecting the points).
			/// This is the default value for the <see cref="FastLine.Color"/> property.
			/// </summary>
			...
			/// <summary>
			/// The default mode for displaying line segments (<see cref="FastLine.IsVisible"/>
			/// property).  True to show the line segments, false to hide them.
			/// </summary>
			public static bool IsVisible = true;
			/// <summary>
			/// The default width for line segments (<see cref="FastLine.Width"/> property).
			/// Units are points (1/72 inch).
			/// </summary>
			public static float Width = 1;
			// Added by BioRad
			/// <summary>
			/// Width when line is highlighted.
			/// </summary>
			public static float HighlightedWidth = 2;
			// end BioRad
			/// <summary>
			/// The default value for the <see cref="FastLine.IsAntiAlias"/>
			/// property.
			/// </summary>
			public static bool IsAntiAlias = false;
			/// <summary>
			/// The default value for the <see cref="FastLine.IsSmooth"/>
			/// property.
			/// </summary>
			public static bool IsSmooth = false;
			/// <summary>
			/// The default value for the <see cref="FastLine.SmoothTension"/> property.
			/// </summary>
			public static float SmoothTension = 0.5F;
			/// <summary>
			/// The default drawing style for line segments (<see cref="FastLine.Style"/> property).
			...
			/// (<see cref="FastLine.StepType"/>).  This determines if the curve
		...
		#region Properties
		/// <summary>
		/// The color of the <see cref="FastLine"/>
		/// </summary>
		...
		/// <summary>
		/// The style of the <see cref="FastLine"/>, defined as a <see cref="DashStyle"/> enum.
		...
		/// <summary>
		/// The pen width used to draw the <see cref="FastLine"/>, in points (1/72 inch)
		/// </summary>
		...
		// BioRad
		/// <summary>
		/// Width of line when it is highlighted.
		/// </summary>
		public float HighlightedWidth
		{
			get { return _highlightedWidth; }
			set { _highlightedWidth = value; }
		}
		// End BioRad
		/// <summary>
		/// Gets or sets a property that shows or hides the <see cref="FastLine"/>.
		/// </summary>
		...
		/// this <see cref="FastLine" /> is drawn.  A value of false will leave the value of
		...
		/// <summary>
		/// Gets or sets a property that determines if this <see cref="FastLine"/>
		...
		/// for this <see cref="FastLine"/>.  This property is only used if
		...
		#region Constructors
		/// <summary>
		/// Default constructor that sets all <see cref="FastLine"/> properties to default
		...
		/// the remaining <see cref="FastLine"/> properties to default
		/// values as defined in the <see cref="Default"/> class.
		/// </summary>
		/// <param name="color">The color to assign to this new FastLine object</param>
		public FastLine(Color color)
		{
			_width = Default.Width;
			// BioRad
			_highlightedWidth = Default.HighlightedWidth;
			// End BioRad
		...
		/// </summary>
		/// <param name="rhs">The FastLine object from which to copy</param>
		public FastLine(FastLine rhs)
		{
			_width = rhs._width;
			// BioRad
			_highlightedWidth = rhs._highlightedWidth;
			// End BioRad
			_style = rhs._style;
		...
		public FastLine Clone()
		{
			return new FastLine(this);
		}
		...
		/// </param>
		protected FastLine(SerializationInfo info, StreamingContext context)
		{
		...
		/// <summary>
		/// Do all rendering associated with this <see cref="FastLine"/> to the specified
		/// <see cref="Graphics"/> device.  This method is normally only
		/// called by the Draw method of the parent <see cref="FastLineItem"/> object.
		/// </summary>
		/// <param name="g">
		...
		/// </param>
		/// <param name="curve">A <see cref="FastLineItem"/> representing this
		/// curve.</param>
		...
		/// <summary>
		/// Render a single <see cref="FastLine"/> segment to the specified
		...
		/// <summary>
		/// Render the <see cref="FastLine"/>'s as vertical sticks (from a <see cref="StickItem" />) to
		...
		/// device using the specified smoothing property (<see cref="ZedGraph.FastLine.SmoothTension"/>).
		...
		/// is ignored for smooth lines (e.g., when <see cref="ZedGraph.FastLine.IsSmooth"/> is true).
		/// </summary>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		public void DrawCurve(Graphics g, GraphPane pane, CurveItem curve, float scaleFactor)
		{
			...
			float unscaledWidth = _width;
			if (curve.IsHighlighted)
				unscaledWidth = HighlightedWidth;
			float modifiedWidth = pane.ScaledPenWidth(unscaledWidth, scaleFactor);
			Pen pen = new Pen(_color, modifiedWidth);
			pen.DashStyle = this.Style;

			if (points != null && !_color.IsEmpty && this.IsVisible)
			{
				List<PointF> linePoints = new List<PointF>(points.Count);
				...
				for (int i = 0; i < points.Count; i++)
				{
					...
					{
						// Transform the current point from user scale units to screen coordinates
						//const double minLog = Scale.Default.MinLogValue;
						//if (yIsLog && curY < minLog)
						//   curY = minLog;
						//if (xIsLog && curX < minLog)
						//   curX = minLog;
						...
						{
							linePoints.Add(new PointF(tmpX, tmpY));
						}
				...
				if (linePoints.Count >= 2)
				{
					if (curve.IsHighlighted)
					{
						Pen highlightPen = new Pen(Color.Black, modifiedWidth + 4);
						highlightPen.DashStyle = this.Style;
						g.DrawLines(highlightPen, linePoints.ToArray());
					}
		...
		/// the current curve.  Note that this drawing routine ignores see PointPair.Missing
		...
		/// </summary>
		/// <remarks>Note that this drawing routine ignores see PointPair.Missing
		/// values, but it does not "break" the line to indicate values are missing.
		/// </remarks>
		/// <param name="pane">A reference to the <see cref="GraphPane"/> object that is the parent or
		/// owner of this object.</param>
									...
									int count, double yMin, GraphicsPath path)
		{
			...
			else
			{
				...
				// Find the next lower curve in the curveList that is also a FastLineItem type, and use
				...
				if (index > 0)
				{
					...
					for (int i = index - 1; i >= 0; i--)
					{
						...
						if (tmpCurve is FastLineItem)
						{
							tension = ((FastLineItem)tmpCurve).Line.IsSmooth ? ((FastLineItem)tmpCurve).Line.SmoothTension : 0f;
							break;
						}
					}
				}
