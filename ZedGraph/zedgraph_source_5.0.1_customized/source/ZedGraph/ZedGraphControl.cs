...
namespace ZedGraph
{
	...
	/// <version> $Revision: 11 $ $Date: 5/14/10 11:21a $ </version>
	public partial class ZedGraphControl : UserControl
	{
		...
		/// This format is passed to PointPair.ToString(string).
		...
		*/
		...
		/// if it is a <see cref="System.String"/> type, or see PointPair.ToString()
		...
		/// This format is passed to see PointPair.ToString(string).
		/// </summary>
		/// <remarks>
		...
		{
			// BioRad addition 2007.3.22 : a quick-out so we can handle the cursor from a higher level container control.
			return;
			// End BioRad addition.

			// BioRad addition 2007.3.22 : added #if false region to avoid unreachable code warning.
#if false
			// End BioRad addition.
			if ( _masterPane != null )
			{
		...
		protected string MakeValueLabel(Axis axis, double val, int iPt, bool isOverrideOrdinal)
		{
			if (axis != null)
			{
				...
				else
				{
					...
					return DoubleToStringWithMinimumSignificantDigits(val, 4);
					// end biorad change
				}
			}
			else
		...
		// Added by BioRad
		/// <summary>
		/// returns a format string which, when used with the ToString of the passed in value, yields a string representation
		/// which includes the given minimum significant digits.  This does not use scientific notation.
		/// </summary>
		/// <param name="value"></param>
		/// <param name="minimumSignificantDigits"></param>
		/// <returns></returns>
		private string DoubleToStringWithMinimumSignificantDigits(double value, int minimumSignificantDigits)
		{
			int mostSignificantDigitIndex;
			if (value == 0)
				mostSignificantDigitIndex = 0;
			else
			{
				double log = Math.Log10(Math.Abs(value));
				// compute index of most significant digit - 0 is the 1s digit, 1 is the 10s digit.  -1 is the 10ths digit, and so on.
				if (log >= 0)
					mostSignificantDigitIndex = (int)Math.Floor(log);
				else
					mostSignificantDigitIndex = (int)Math.Ceiling(Math.Abs(log)) * -1;
			}
			int decimalPlacesNeededForSigDigits = 0;
			if (mostSignificantDigitIndex + 1 < minimumSignificantDigits)
				decimalPlacesNeededForSigDigits = mostSignificantDigitIndex * -1 + minimumSignificantDigits - 1;

			int decimalPlaces = decimalPlacesNeededForSigDigits;
			char[] formatChars = new char[decimalPlaces + 2];
			for (int i = 0; i < formatChars.Length; i += 1)
				formatChars[i] = '0';
			formatChars[1] = '.';
			return value.ToString(new string(formatChars));
		}
		// end Biorad addition.
					...
					g, out pane, out nearestObj, out iPt))
				{
					if (nearestObj is CurveItem && iPt >= 0)
					{
						...
						else
						{
							...
							{
								IPointPair pt = curve.Points[iPt];
								...
								else
								{
									...
									string toolTip = "( " + xStr + ", " + yStr + " )";
									if (pointToolTip.GetToolTip(this) != toolTip)
										this.pointToolTip.SetToolTip(this, toolTip);
								}
							}
...
#endif
			// End bioRad.  (Quick-out added, compiled out the guts of the method to avoid unreachable code warning.)
		}
		...
		private void HandleZoomDrag(Point mousePt)
		{
			...
			if (newWay)
			{
				...
				DragRect.DrawDragRect(_dragStartPt, _dragEndPt, 1, this);
		...
		private void HandleZoomFinish(object sender, MouseEventArgs e)
		{
					...
					(Math.Abs(mousePtF.Y - _dragStartPt.Y) > 4 || !_isEnableVZoom))
			{
				...
				if (_isEnableHZoom)
				{
					_dragPane.XAxis._scale._forcedMin = Math.Min(x1, x2);
					_dragPane.XAxis._scale._minAuto = false;

					_dragPane.XAxis._scale._forcedMax = Math.Max(x1, x2);
				...
				if (_isEnableVZoom)
				{
					for (int i = 0; i < y1.Length; i++)
					{
						_dragPane.YAxisList[i]._scale._forcedMin = Math.Min(y1[i], y2[i]);
						_dragPane.YAxisList[i]._scale._forcedMax = Math.Max(y1[i], y2[i]);
						_dragPane.YAxisList[i]._scale._minAuto = false;
						_dragPane.YAxisList[i]._scale._maxAuto = false;
					}
					for (int i = 0; i < yy1.Length; i++)
					{
						_dragPane.Y2AxisList[i]._scale._forcedMin = Math.Min(yy1[i], yy2[i]);
						_dragPane.Y2AxisList[i]._scale._forcedMax = Math.Max(yy1[i], yy2[i]);
		...
		private void Synchronize(Axis source, Axis dest)
		{
			...
			dest._scale._forcedMin = source._scale._forcedMin;
			dest._scale._forcedMax = source._scale._forcedMax;
		...
		public void SetScrollRangeFromData()
		{
			if (this.GraphPane != null)
			{
				...
				if (this.GraphPane.XAxis.Scale.IsAnyOrdinal)
				{
					_xScrollRange.Min -= .5;
					_xScrollRange.Max += .5;
				}
		...
		private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
		{
			...
			if (_masterPane != null && menuStrip != null)
			{
				...
				if (_isShowContextMenu)
				{
					...
					item.Click += new System.EventHandler(this.MenuClick_SaveForPublication);
					menuStrip.Items.Add(item);

					//item = new ToolStripMenuItem();
					//item.Name = "save_as_publish";
					//item.Tag = "save_as_publish";
					//item.Text = _resourceManager.GetString("export_for_publication");
					//item.Click += new System.EventHandler(this.MenuClick_SaveForPublication);
					//menuStrip.Items.Add(item);
					...
					// BioRad changes 6.18.2007 : removed two context menu items
					//item = new ToolStripMenuItem();
					//item.Name = "unzoom";
					//item.Tag = "unzoom";
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		protected void MenuClick_SaveForPublication(System.Object sender, System.EventArgs e)
		{
			Form parent = ParentForm; 
			if (parent != null && parent.IsMdiChild)
				parent = ParentForm.MdiParent;
			using (ExportForPublicationDialog d = new ExportForPublicationDialog(parent, _masterPane))
			{
		...
		*/
		...
		public void DoPageSetup()
		{
            ...
            {
                MessageBox.Show(_resourceManager.GetString("no_printer_installed"));
                return;
            }
	...
	/// <summary>Utility class which exposes functionality to draw an XOR rectangle ("rubber-band box") onto Control.  
	/// This is meant to be a replacement for ControlPaint.DrawReversibleFrame(...), which does not work correctly in 
	/// Vista.</summary>
	public class DragRect
	{
		private const uint c_PatInvert = (uint)0x005A0049;
		...
		/// <summary>Draws an XOR rectangle ("rubber-band box") onto the given control.  This is meant to be
		/// a replacement for ControlPaint.DrawReversibleFrame(...), which does not work correctly in Vista.</summary>
		/// <param name="point1"></param>
		/// <param name="point2"></param>
		/// <param name="lineThickness"></param>
		/// <param name="c"></param>
		public static void DrawDragRect(Point point1, Point point2, int lineThickness, Control c)
		{
			using (Graphics grfxClient = c.CreateGraphics())
			{
				DrawDragRectAux(ref point1, ref point2, lineThickness, grfxClient);
			}
		}

		private static void DrawDragRectAux(ref Point point1, ref Point point2, int lineThickness, Graphics g)
		{
			List<int> Xs = new List<int>(new int[] { point1.X, point2.X });
			List<int> Ys = new List<int>(new int[] { point1.Y, point2.Y });
			Xs.Sort();
			Ys.Sort();
			point1 = new Point(Xs[0], Ys[0]);
			point2 = new Point(Xs[1], Ys[1]);
			Size sz = new Size(point2.X - point1.X, point2.Y - point1.Y);

			Rectangle outerRect = new Rectangle(point1, sz);
			Region outerRgn = new Region(outerRect);
			Rectangle innerRect = new Rectangle(point1, sz);
			innerRect.Inflate(-lineThickness, -lineThickness);
			Region innerRgn = new Region(innerRect);
			outerRgn.Xor(innerRgn);

			IntPtr hBr = CreateSolidBrush(RGB(255, 255, 255));
			IntPtr hrgn = outerRgn.GetHrgn(g);
			IntPtr hDc = g.GetHdc();

			SelectClipRgn(hDc, hrgn);
			IntPtr hOldBrush = SelectObject(hDc, hBr);

			PatBlt(hDc, outerRect.Left, outerRect.Top, outerRect.Width, outerRect.Height, c_PatInvert);

			SelectObject(hDc, hOldBrush);
			DeleteObject(hBr);
			g.ReleaseHdc();
			innerRgn.ReleaseHrgn(hrgn);
