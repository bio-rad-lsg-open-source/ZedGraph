...
{
	/// <summary>
	/// The Log2Scale class inherits from the <see cref="Scale" /> class, and implements
	/// the features specific to <see cref="AxisType.Log2" />.
	/// </summary>
	/// <remarks>
	/// Log2Scale is a non-linear axis in which the values are scaled using the base 2
	/// <see cref="Math.Log(double)" />
	/// function.
	/// 
	/// This file was added by BioRad on 2007.01.01 and is based on LogScale.cs.
	/// </remarks>
	/// 
	/// <author> John Champion  
	/// modified by BioRad</author>
	/// <version> $Revision: 5 $ $Date: 7/08/09 4:45p $ </version>
	[Serializable]
	class Log2Scale : Scale, ISerializable //, ICloneable
	{
	...
	#region constructors
		...
		public Log2Scale( Scale rhs, Axis owner )
			: base( rhs, owner )
		{
		}
		...
		{
			return new Log2Scale( this, owner );
		}
	...
	#region properties
		...
		{
			get { return AxisType.Log2; }
		}

		// begin biorad change: removed overrides, MinLogDisplayValue takes care of this instead.
#if false
		/// <summary>
	...
	#region methods
		...
		/// </param>
		/// <param name="iTic">
		/// The major tic number (0 = first major tic).  For log scales, this is the actual power of 2.
		/// </param>
		/// <returns>
		...
		override internal double CalcMinorTicValue( double baseVal, int iTic )
		{
			...
			return baseVal + 1;// +Math.Floor((double)iTic / 9.0) + dLogVal[(iTic + 9) % 9];
		}
		...
		override internal double CalcBaseTic()
		{
			...
			else
			{
				...
				return Math.Ceiling( SafeLog( _min, 2 ) - 0.00000001 );
			}
		...
		override internal int CalcNumTics()
		{
			...
			nTics = (int) ( Math.Floor( SafeLog( _max, 2 ) + 1.0e-12 ) ) -
					(int) ( Math.Ceiling( SafeLog( _min, 2 ) - 1.0e-12 ) ) + 1;
		...
		/// <summary>
		/// Select a reasonable base 2 logarithmic axis scale given a range of data values.
		/// </summary>
		/// <remarks>
		...
		/// based always on powers of 2 (full log cycles).  This
		...
		override public void PickScale( GraphPane pane, Graphics g, float scaleFactor )
		{
			...
			if (_minAuto)
			{
				if (_min <= MinLogDisplayValue)
				{
					_min = MinLogDisplayValue;
				}
				if (_max <= MinLogDisplayValue)
				{
					_max = _min * 10.0;
				}
			}
			...
			if (_minAuto)
				_min = Math.Pow((double)2.0,
					//	Math.Floor( 
					Math.Log(_min, 2)
					//) 
					);
			if (_maxAuto)
				_max = Math.Pow((double)2.0,
					//Math.Ceiling( 
					Math.Log(_max, 2)
					//) 
					);
		...
		override internal string MakeLabel( GraphPane pane, int index, double dVal )
		{
			...
			if ( _isUseTenPower )
				...
				return Math.Pow( 2.0, dVal ).ToString( _format );
		}
	...
	#region Serialization
		...
		/// </param>
		protected Log2Scale( SerializationInfo info, StreamingContext context ) : base( info, context )
		{
