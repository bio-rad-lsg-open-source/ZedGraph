...
namespace ZedGraph
{
	...
	/// <version> $Revision: 3 $ $Date: 5/14/10 11:20a $ </version>
	[Serializable]
	public class PointPairList : List<IPointPair>, IPointList, IPointListEdit
	{
	#region Fields
...
*/
	...
	#region Methods
		...
		{
			foreach ( IPointPair point in pointList )
				Add( point );
		...
		/// If the arrays are of different size, then the larger array prevails and the
		/// smaller array is padded with see PointPair.Missing values.
		/// </summary>
		/// <param name="x">A double[] array of X values</param>
		/// <param name="y">A double[] array of Y values</param>
		...
		{
			_sorted = false;
			base.Insert( index, new PointPair2D( x, y ) );
		}
		...
		*/
		...
		/// see PointPair.Equals method.</remarks>
		...
		/// see PointPair.Missing if extrapolation would be required).  WARNING: Cardinal
