...
namespace ZedGraph
{
	...
	/// <version> $Revision: 6 $ $Date: 6/17/10 11:54a $ </version>
	[Serializable]
	...
	#region Font Construction Methods
		...
		private void Remake( float scaleFactor, float size, ref float scaledSize, ref Font font )
		{
			...
			// Added by BioRad 9.19.2007
			if (_isBold)
				newSize = Math.Max(newSize, 10.5f); // Bold doesn't work at font sizes less than this, at least for most fonts.
			// End BioRad addition
					...
					font.Underline != _isUnderline )
			{
				...
				{
					font = new Font(FontFamily.GenericSansSerif, scaledSize, style, GraphicsUnit.World);
				}
			}
		}
	...
	#region Rendering Methods
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		/// <param name="powerBase">base</param>
		/// <remarks>Changed by BioRad: Changed name from DrawTenPower to DrawPower, and
		/// added the powerBase parameter, so the method works with any base.</remarks>
		public void DrawPower( Graphics g, GraphPane pane, string text, float x,
			float y, AlignH alignH, AlignV alignV,
			float scaleFactor, int powerBase )
		{
			SmoothingMode sModeSave = g.SmoothingMode;
			if ( _isAntiAlias )
				g.SmoothingMode = SmoothingMode.HighQuality;
			...
			SizeF sizePowerBase = g.MeasureString( powerBase.ToString(), _font );
			SizeF sizeText = g.MeasureString( text, _superScriptFont );
			SizeF totSize = new SizeF( sizePowerBase.Width + sizeText.Width,
									sizePowerBase.Height + sizeText.Height * Default.SuperShift );
			...
			// is set up such that 0,0 is at the location where the
			// CenterTop of the text needs to be.
			g.DrawString( powerBase.ToString(), _font, brush,
							( -totSize.Width + sizePowerBase.Width ) / 2.0F,
	...
	#region Sizing Methods
				...
				AlignV alignV, float angle )
		{
			...
			// parameters
			newMatrix.Translate(xa, ya, MatrixOrder.Append);

			// Rotate the coordinate system according to the 
			// specified angle of the FontSpec
			if (Angle != 0.0F)
				newMatrix.Rotate(-angle, MatrixOrder.Append);

			// Move the coordinate system to local coordinates
			// of this text object (that is, at the specified
			// x,y location)
			newMatrix.Translate(x, y, MatrixOrder.Append);

			// Append the passed-in matrix transform, which is a translation, to the new one.
