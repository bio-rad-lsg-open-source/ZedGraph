...
namespace ZedGraph
{
	...
	/// symbols at each point.  Optimized for drawing speed.
	/// </summary>
	/// 
	/// <author> John Champion 
	/// This file was added by BioRad.  It is based on LineItem.cs.</author>
	/// <version> $Revision: 4 $ $Date: 5/14/10 11:20a $ </version>
	[Serializable]
	public class FastLineItem : CurveItem, ILineItem, ICloneable, ISerializable
	{
		#region Fields

		/// <summary>
		/// Private field that stores a reference to the <see cref="ZedGraph.Line"/>
		/// class defined for this <see cref="FastLineItem"/>.  Use the public
		/// property <see cref="Line"/> to access this value.
		/// </summary>
		protected FastLine _line;
		...
		/// for this <see cref="FastLineItem"/>.
		/// </summary>
		...
		#region Constructors
		/// <summary>
		/// Create a new <see cref="FastLineItem"/>, specifying only the legend <see cref="CurveItem.Label" />.
		/// </summary>
		/// <param name="label">The _label that will appear in the legend.</param>
		public FastLineItem(string label)
			: base(label)
		{
			//_symbol = new Symbol();
			_line = new FastLine();
		...
		/// <summary>
		/// Create a new <see cref="FastLineItem"/> using the specified properties.
		/// </summary>
		/// <param name="label">The _label that will appear in the legend.</param>
		...
		/// type of symbol to use for this <see cref="FastLineItem"/>.  Use <see cref="SymbolType.None"/>
		...
		public FastLineItem(string label, double[] x, double[] y, Color color, SymbolType symbolType, float lineWidth)
		...
		/// type of symbol to use for this <see cref="FastLineItem"/>.  Use <see cref="SymbolType.None"/>
		/// to hide the symbols.</param>
		public FastLineItem(string label, double[] x, double[] y, Color color, SymbolType symbolType)
		...
		/// width is scaled based on <see cref="PaneBase.CalcScaleFactor"/>.  Use a value of zero to
		/// hide the line (see <see cref="ZedGraph.Line.IsVisible"/>).</param>
		public FastLineItem(string label, IPointList points, Color color, SymbolType symbolType, float lineWidth)
			: base(label, points)
		{
			_line = new FastLine(color);
			if (lineWidth == 0)
		...
		/// type of symbol to use for this <see cref="FastLineItem"/>.  Use <see cref="SymbolType.None"/>
		/// to hide the symbols.</param>
		public FastLineItem(string label, IPointList points, Color color, SymbolType symbolType)
		...
		/// </summary>
		/// <param name="rhs">The <see cref="FastLineItem"/> object from which to copy</param>
		public FastLineItem(FastLineItem rhs)
			: base(rhs)
		{
			//_symbol = new Symbol(rhs.Symbol);
			_line = new FastLine(rhs.Line);
		}
		...
		public FastLineItem Clone()
		{
			return new FastLineItem(this);
		}
		...
		/// </param>
		protected FastLineItem(SerializationInfo info, StreamingContext context)
			: base(info, context)
		{
			...
			_line = (FastLine)info.GetValue("line", typeof(FastLine));
		}
		/// <summary>
		...
		#region Methods
		/// <summary>
		/// Do all rendering associated with this <see cref="FastLineItem"/> to the specified
		...
		/// <summary>
		/// Draw a legend key entry for this <see cref="FastLineItem"/> at the specified location
		/// </summary>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		/// <summary>
		/// Loads some pseudo unique colors/symbols into this FastLineItem.  This
		/// is mainly useful for differentiating a set of new FastLineItems without
