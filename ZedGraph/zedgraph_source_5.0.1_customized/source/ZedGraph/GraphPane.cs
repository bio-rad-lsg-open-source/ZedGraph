...
namespace ZedGraph
{
	...
	/// <version> $Revision: 3 $ $Date: 2/28/07 6:41p $ </version>
	[Serializable]
	public class GraphPane : PaneBase, ICloneable, ISerializable
	{
		...
		/// see PointPair.Missing values will cause the line segments of
		...
		/// <see cref="AddFastCurve(string,double[],double[],Color)"/> method.</returns>
		public FastLineItem AddFastCurve(string label, double[] x, double[] y, Color color)
		{
			FastLineItem curve = new FastLineItem(label, x, y, color, SymbolType.Default);
		...
		/// <param name="label">The text label (string) for the curve that will be
		/// used as a <see cref="Legend"/> entry.</param>
		/// <param name="ppl">curve points</param>
		/// <param name="color">The color to used for the curve line,
		/// symbols, etc.</param>
		/// <returns>A <see cref="CurveItem"/> class for the newly created curve.
		/// This can then be used to access all of the curve properties that
		/// are not defined as arguments to the
		/// <see cref="AddFastCurve(string,double[],double[],Color)"/> method.</returns>
		public void AddFastCurve(string label, PointPairList ppl, Color color)
		{
			FastLineItem curve = new FastLineItem(label, ppl, color, SymbolType.Default);
			_curveList.Add(curve);
		}
		/// <summary>
		/// Add a curve (<see cref="CurveItem"/> object) to the plot with
